...
{
    class AegisBornPlayer : IOperationHandler
    {
        ...
        static AegisBornPlayer()
        {
            Operations = new OperationMethodInfoCache();
            try
            {
                Operations.RegisterOperations(typeof(AegisBornPlayer));
            }
            catch (Exception e)
            {
        ...
        public AegisBornPlayer(Peer photonPeer, SfGuardUser user, AegisBornCharacter selectedCharacter)
        {
            _user = user;
        ...
        public void ExitGame()
        {
            ...
            _peer.SetCurrentOperationHandler(new AegisBornAccount(_peer, _user));
