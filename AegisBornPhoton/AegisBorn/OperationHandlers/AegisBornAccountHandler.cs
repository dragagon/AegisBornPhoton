...
namespace AegisBorn.OperationHandlers
{
    public class AegisBornAccountHandler : IOperationHandler
    {
        ...
        static AegisBornAccountHandler()
        {
            Operations = new OperationMethodInfoCache();
            try
            {
                Operations.RegisterOperations(typeof(AegisBornAccountHandler));
            }
            catch (Exception e)
            {
        ...
        public AegisBornAccountHandler(Peer photonPeer, SfGuardUser user)
        {
            _user = user;
            _peer = photonPeer;
